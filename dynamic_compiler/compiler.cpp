...
namespace dynamic_compiler {
    bool Compiler::generate_bin(const std::vector<std::string> &cmds, const std::string outputFilename, hhal::Unit arch) const {
    ...
    const std::string Compiler::get_binary(const std::string source, hhal::Unit arch) const {
        ...
        if (should_compile) {
            ...
            bool success;
            if(generated_entry)
                success = generate_bin({source, entry_path, "-I/opt/mango/usr/include/libmango/", "-L/opt/mango/usr/lib", "-pthread", "-lmango-dev-gn"}, bin_path, arch);
            else
                success = generate_bin({source, "-I/opt/mango/usr/include/libmango/", "-L/opt/mango/usr/lib", "-pthread", "-lmango-dev-gn"}, bin_path, arch);
