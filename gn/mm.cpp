...
void MM::set_tlb_kb(GNManager &manager, mango_id_t unit, mango_id_t mem_bank, mango_addr_t starting_addr,
    ...
    if (ec != GNManagerExitCode::OK) {
        printf("[Error] Setting TLB for cluster %d, unit %d, bank %d, addr %d, size %d, phy_addr %d FAILED\n", cluster_id, unit, mem_bank, starting_addr, size, phy_addr);
    }
}
...
mango_exit_code_t MM::set_vaddr_kernels(GNManager &manager, std::vector<gn_kernel> &kernels) noexcept {
    ...
    for(auto &k : kernels) {
        ...
        printf("[Notice] Mapped kernel image. [tile=%d, mem_bank=%d, phy_addr=%p, "
                    "virt_addr_kernel=%p, size=%d]\n", tile_unit, mem_bank, phy_addr,
...
mango_exit_code_t MM::set_vaddr_events(GNManager &manager, std::vector<gn_event> &events) noexcept {
    ...
    for(auto& k_pair : manager.kernel_info) {
        ...
        printf("[Notice] Configured TLB for events of tile %d [%p - %p]\n", tile_unit, start_addr, end_addr);
...
void MM::set_buff_tlb(GNManager &manager, gn_kernel &k, gn_buffer &b) noexcept {
    ...
    printf("[Notice] Mapped buffer %d to kernel %d [virt_addr=%p] [entry=%d]\n",
